...
export const getBooks = async (req, res) => {
    ...
    try {
        ...
        if(userRole.type === "Seller"){
            const books = await BookModel.find({authorId:userID}).populate('review');
...
export const updateBook = async (req, res) => {
    try {
        ...
        if(user.type !== "Seller" || user.type !== "Admin"){
            return res.status(403).json({ success: false, message: "Only sellers or admin can update books" });
        }
        ...
        const updatedBook = await BookModel.findByIdAndUpdate({authorId:authorId, _id:id}, updates, { new: true });
      
        if (!updatedBook) {
            return res.status(404).json({ success: false, message: "Book not found" });
        }
        return res.status(200).json({ success: true, message: "Book updated successfully", book: updatedBook });
    }
    catch (error) {
...
export const deleteBook = async (req, res) => {
    try {
        ...
        const deletedBook = await BookModel.findByIdAndDelete({authorId, _id:id});
        if (!deletedBook) {
            return res.status(404).json({ success: false, message: "Book not found" });
        }
...
export const rate = async(req,res)=>{
    try{
        ...
        if(!bookId || !rating || !comment){
            return res.status(400).json({success:false,message:"bookId, rating and comment are required"});
        }
        ...
        const existingReview =await ReviewModel.findOne({bookId:bookId,authorId});
